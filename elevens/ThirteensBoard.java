	...
	private static final int BOARD_SIZE = 9;
	  ...
	  public boolean isLegal(List<Integer> paramList)
	  {
	    ...
	    if (paramList.size() == 2)
	    {
	      ...
	      return cardAt(i).pointValue() + cardAt(j).pointValue() == 13;
	    }
	  ...
	  {
	    return (containsPairSum11()) || (containsJQK()) || (containsK());
	  }
	  ...
	  public boolean containsPairSum11()
	  {
	    for (int i = 0; i < 9; i++) {
	      if (cardAt(i) != null) {
	        for (int j = i + 1; j < 9; j++) {
	          if ((cardAt(j) != null) && (cardAt(i).pointValue() + cardAt(j).pointValue() == 13)) {
	            return true;
	          }
	        }
	      }
	  ...
	  public boolean containsK() {
		  ...
		  return containsK(localArrayList);
	  }
	  ...
	  public boolean containsK(List<Integer> paramList) {
		  for(Integer localInteger : paramList) {
		  int m = localInteger.intValue();
		  if(cardAt(m).rank().equals("king")) {
			  return true;
		  }
		  }
		  return false;
	  ...
	  public void processPairSum11()
	  {
	    ...
	    for (int i = 0; i < 9; i++) {
	      if (cardAt(i) != null) {
	        for (int j = i + 1; j < 9; j++) {
	          if ((cardAt(j) != null) && 
	            (cardAt(i).pointValue() + cardAt(j).pointValue() == 13))
	          {
	            localArrayList.add(new Integer(i));
